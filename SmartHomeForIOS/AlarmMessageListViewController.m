...
- (void)viewDidLoad {
    ...
    // 2016 01 25 start
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(refreshDataMessage) name:@"alarmlistrefreshdata" object:nil];
    // 2016 01 25 end
...
{
    NSLog(@"refreshDataByAlarmMessage");
    //
    NSString *msgId = @"0";
    ...
    [DeviceNetworkInterface getAlarmListWithMsgId:msgId andMsgCnt:msgCnt withBlock:^(NSString *result, NSString *message, NSArray *alarms, NSError *error) {
        if (!error) {
            if ([result isEqualToString:@"success"]) {
                ...
                UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"消息" message:message delegate:self cancelButtonTitle:@"OK" otherButtonTitles: nil];
                [alert show];
            }
        }
        else{
...
//    NSLog(@"sdfsdf==%@",[self.rowsForSection objectForKey:self.sections[indexPath.section]][indexPath.row]);
    ...
    if ([rowData[@"type"]isEqualToString:@"0"]) {
        cell.labelTitle.text = definedAlarmType01;
    }else{
        cell.labelTitle.text = rowData[@"type"];
    }
    
    cell.labelSubTitle.text = [NSString stringWithFormat:@"来自%@",rowData[@"devname"]];
//    cell.labelTime.text = rowData[@"datetime"];
    ...
    //2016 01 11 hgc start
//    [cell.imageSnapshot setImage:[self getImageFromURL:rowData[@"snapshotUrl"]]];
    
//    MKNetworkEngine *engine = [[MKNetworkEngine alloc]init];
//    MKNetworkOperation *op = [engine imageAtURL:rowData[@"snapshotUrl"] completionHandler:^(UIImage *fetchedImage, NSURL *url, BOOL isInCache) {
//        [cell.imageSnapshot setImage:fetchedImage];
//    } errorHandler:^(MKNetworkOperation *completedOperation, NSError *error) {
//        [cell.imageSnapshot setImage:[UIImage imageNamed:@"video_icon"]];
//    } ];
//    [engine enqueueOperation:op];
    
//    [UIImageView setDefaultEngine:engine];
//    [cell.imageSnapshot setImageFromURL:rowData[@"snapshotUrl"] placeHolderImage:[UIImage imageNamed:@"video_icon_camera"]];
    //2016 01 15
    
//    NSLog(@"base64string == %@",[CameraTestBase64Code textFromBase64String:rowData[@"snapshotUrl"]]);
    
    //hgc test 2016 01 20
    MKNetworkEngine *engine = [[MKNetworkEngine alloc]init];
    MKNetworkOperation *operation = [engine operationWithURLString:rowData[@"snapshotUrl_S"] params:nil httpMethod:@"GET"];
    //添加完成处理程序
    [operation addCompletionHandler:^(MKNetworkOperation *completedOperation) {
        //请求成功,为_imgView添加图片
        UIImage * testImageMK = [UIImage imageWithData:[completedOperation responseData]];
        //
        [cell.imageSnapshot setImage:testImageMK];
        //
...
//        UIImage *image = [self getImageFromURL:rowData[@"snapshotUrl"]];
////        [cell.imageSnapshot setImage:image];
...
-(UIImage *) getImageFromURL:(NSString *)fileURL {
    ...
    UIImage *placeholder = [UIImage imageNamed:@"video_icon_camera"];
