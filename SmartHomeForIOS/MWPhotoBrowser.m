...
//    [forscreenBtn setBackgroundColor:[UIColor clearColor]];
    ...
    self.tableviewMissBtn = btn;
    [self.view addSubview:btn];
    btn.transform = CGAffineTransformMakeScale(0.001, 0.001);
    [btn addTarget:self action:@selector(tableViewMiss) forControlEvents:UIControlEventTouchUpInside];
...
- (void)setUpTableView
{
    ...
    CGFloat tableViewX = self.view.frame.size.width * 0.5 - 108;
    CGFloat tableViewY = (self.view.frame.size.height - tableViewH) * 0.5;
...
- (void)setUpRetryView
{
    ...
    CGFloat reTryViewX = self.view.frame.size.width * 0.5 - 216 * 0.5;
    CGFloat reTryViewY = self.view.frame.size.height * 0.5 - 110 * 0.5;
    ...
    CGFloat cancelButtonX = CGRectGetMaxX(reTryView.frame);
...
- (void)setUpDlna
{
    ...
    UIView *photoDlnaView = [[[NSBundle mainBundle] loadNibNamed:@"photoDlnaView" owner:self options:nil] objectAtIndex:0];
    photoDlnaView.frame = self.dlnaView.frame;
    [self.dlnaView addSubview:photoDlnaView];
    ...
    self.curIndex = _currentPageIndex;
    if ([self.renderer setAVTransportURI:[app.picUrl objectAtIndex:self.curIndex]]) {
...
{
    self.tableView.transform = CGAffineTransformMakeScale(0.001, 0.001);
    self.tableviewMissBtn.transform = CGAffineTransformMakeScale(0.001, 0.001);
...
//上一个方法
- (IBAction)prevButtonDidPush:(UIButton *)sender
{
    AppDelegate *app = [[UIApplication sharedApplication] delegate];
...
//下一个方法
- (IBAction)nextButtondidPush:(UIButton *)sender
{
    AppDelegate *app = [[UIApplication sharedApplication] delegate];
    NSInteger currentIndex = self.curIndex;
    NSInteger nextIndex = ++currentIndex;
...
- (void)showTableView{
    ...
    self.tableviewMissBtn.transform = CGAffineTransformMakeScale(1, 1);
