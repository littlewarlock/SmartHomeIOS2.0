...
@property Boolean isCameraListFull;
...
- (void)viewWillDisappear:(BOOL)animated{
    NSLog(@"hello helloviewWillDisappear");
...
//honban  20151014hgc
    ...
    if ([rowData[@"fulling"] isEqualToString:@"1"]) {
        //添加摄像头已满，button设置为不可用
        self.isCameraListFull = YES;
        //
...
{
    if (self.isCameraListFull) {
        NSLog(@"list full");
        UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"添加摄像头" message:@"最多添加4个摄像头" delegate:self cancelButtonTitle:@"确认" otherButtonTitles: nil];
        [alert show];
    }else{
