...
#define FreezableOperationExtension @"backupfrozenoperation"
#define NETWORKCACHE_DEFAULT_DIRECTORY @"NetworkKitCache"
@implementation NSOperationBackupQueue

+ (instancetype)sharedInstance{
    static dispatch_once_t onceToken;
    static NSOperationBackupQueue *instance = nil;
    dispatch_once(&onceToken, ^{
        instance = [[NSOperationBackupQueue alloc] init];
...
-(void) freezeOperations {
    ...
    for(FileDownloadOperation *operation in self.operations) {
        NSString *archivePath = [[[self cacheDirectoryName] stringByAppendingPathComponent:operation.taskInfo.taskId] stringByAppendingPathExtension:FreezableOperationExtension];
