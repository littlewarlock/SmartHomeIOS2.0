...
- (void)loginCheck{
    ...
    [op addCompletionHandler:^(MKNetworkOperation *completedOperation) {
        ...
        if([@"0" isEqualToString:results]){
            ...
            [self.passwordText becomeFirstResponder];
            self.passwordText.text=nil;
        }else if([@"9" isEqualToString:results]){
            ...
            UIAlertView* alert=[[UIAlertView alloc]initWithTitle:@"系统提示" message:@"登录失败。" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles: nil];
            [alert show];
            [self.passwordText becomeFirstResponder];
            self.passwordText.text=nil;
