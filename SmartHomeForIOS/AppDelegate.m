...
- (void)application:(UIApplication *)application didReceiveLocalNotification:(UILocalNotification *)notification
{
    ...
    if (![DeviceNetworkInterface isObjectNULLwith:notification.userInfo[@"snapshot"]]) {
        ...
        // 2016 01 19 hgc
        NSLog(@"base64string == %@",[CameraTestBase64Code textFromBase64String:notification.userInfo[@"snapshot"]]);
        
        alarmMessagePopUpViewController.imageView.image = [UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:[CameraTestBase64Code textFromBase64String:notification.userInfo[@"snapshot"]]]]];
    }else{
    ...
    if (![DeviceNetworkInterface isObjectNULLwith:notification.userInfo[@"type"]]) {
        if ([notification.userInfo[@"type"] isEqualToString:@"0"]) {
            alarmMessagePopUpViewController.labelTitle.text = definedAlarmType01;
        }else{
...
//
    ...
    UITapGestureRecognizer *bottomViewGesture = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(localNotiClose)];
    [alarmMessagePopUpViewController.viewBottomSide addGestureRecognizer:bottomViewGesture];
    ...
    [UIView animateWithDuration:0.5f delay:0.0f options:UIViewAnimationCurveEaseInOut|UIViewAnimationOptionAllowUserInteraction animations:^{
        ...
        if (finished) {
            ...
            if (self.popUpView != NULL ) {
                NSLog(@"not null");
                [self performSelector:@selector(removePopView) withObject:nil afterDelay:5.0f];
            }
        }
    }];
    //
